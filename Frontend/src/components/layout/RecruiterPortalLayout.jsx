      ...
      <div className="relative mx-auto flex w-full max-w-[1600px] h-screen overflow-hidden">
        ...
        <aside className="hidden lg:flex w-72 border-r border-white/5 bg-[#090F1C]/80 backdrop-blur-xl px-5 py-6 flex-col justify-between flex-shrink-0 z-10">
          ...
          <div className="space-y-6">
            {/* Logo */}
            <Link to="/recruiter/dashboard" className="flex items-center gap-3.5 px-2">
              <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-accent/10 border border-accent/20 text-accent shadow-[0_0_15px_rgba(56,189,248,0.25)]">
                <Users2 size={20} />
              </div>
              <div>
                <div className="text-sm font-bold text-white tracking-tight">Intervix</div>
                <div className="text-[9px] uppercase tracking-[0.25em] font-semibold text-gray-500 mt-0.5">Recruiter Board</div>
              </div>
            </Link>

            {/* Navigation items */}
            <nav className="space-y-1.5 pt-4">
              <div className="text-[9px] font-bold uppercase tracking-[0.2em] text-gray-500 mb-3 px-3">Hiring Menu</div>
              {navItems.map((item) => {
                ...
                return (
                  <Link
                    ...
                    className={`flex items-center gap-3.5 rounded-xl px-3.5 py-2.5 text-xs font-semibold transition-all duration-200 ${
                      active 
                        ? 'bg-accent/10 border border-accent/20 text-white shadow-[0_0_12px_rgba(56,189,248,0.15)]' 
          ...
          <div className="space-y-4 pt-6 border-t border-white/5">
            <div className="flex items-center gap-3 p-2 bg-white/[0.01] border border-white/5 rounded-2xl">
              ...
              <div className="min-w-0">
                ...
                <div className="text-[9px] uppercase font-bold text-accent mt-0.5">{user?.role}</div>
              </div>
            </div>
        ...
        <main className="flex-1 flex flex-col min-h-0 overflow-y-auto">
          ...
          <header className="relative border-b border-white/5 bg-[#090F1C]/40 backdrop-blur-xl px-8 py-5 flex items-center justify-between z-40 flex-shrink-0">
            <div>
              <div className="text-[9px] uppercase tracking-[0.2em] font-bold text-gray-500">Recruiter Portal</div>
              <h1 className="mt-1 text-base font-bold text-white">Operate structured live candidate interview suites</h1>
            </div>
